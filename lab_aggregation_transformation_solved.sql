...
-- 2.3 Retrieve rental information and add an additional column called DAY_TYPE with values 'weekend' or 
...
-- BONUS
-- 4. The marketing team for the movie rental company now needs to create a personalized email campaign 
...
-- and use their email address to send personalized recommendations. The results should be ordered by last name 
-- in ascending order to make it easier to use the data

SELECT first_name, last_name, email FROM sakila.customer ORDER BY last_name ASC;
...
SELECT COUNT(rating),rating FROM sakila.film GROUP BY rating;
...
SELECT COUNT(rating),rating FROM sakila.film GROUP BY rating ORDER BY COUNT(rating) DESC;
...
SELECT COUNT(rating), rating, ROUND(AVG(length),2) FROM sakila.film GROUP BY rating ORDER BY AVG(length) DESC;
...
SELECT COUNT(rating), rating, ROUND(AVG(length),2) 
FROM sakila.film 
GROUP BY rating 
HAVING AVG(length) > 119
